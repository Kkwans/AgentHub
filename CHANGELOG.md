...
- 正式 NAS Compose 当前为 `agenthub:0.6.0-nas.42`（revision `b3d1cac`），入口为 `192.168.5.110:3210`，容器 root/privileged、running/healthy；升级未执行 `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器。

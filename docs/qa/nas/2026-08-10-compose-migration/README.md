...
- 认证：唯一管理员账号、用户名/密码与 HttpOnly Cookie；API token 仅供外部集成
...
管理员登录更新前的第二份冷备份位于
`/volume2/Project/.agenthub/central/deployments/20260810T195343-pre-account-auth/`。当前
账号版镜像与恢复证据见
[管理员登录 Compose 更新验收](../2026-08-10-account-auth/README.md)。
...
- 非 loopback 入口强制认证，未登录浏览器只可访问账号状态、首次设置与登录接口；
- root-only API token 仍保存在 `/volume2/DockerProject/agenthub/secrets/browser-token`，仅供
  CLI/外部集成兼容与回滚，文件和父目录均为 root-only，不再是浏览器登录步骤；
...
- WebSocket：浏览器使用 HttpOnly Cookie；API token subprotocol 仅保留兼容路径；
...
- 受控 `docker compose restart agenthub` 后容器恢复 healthy，同一 Project/Target 与
  `setupRequired=true` 的首次设置状态均持久恢复。
...
初次迁移的 token 入口历史证据见
[TX5Pro Compose LAN 验收](../../../tx5pro/2026-08-10-compose-lan/README.md)；现行管理员首次设置
流程的机器可读报告和截图见
[管理员登录 TX5Pro 验收](../../../tx5pro/2026-08-10-account-auth/README.md)。

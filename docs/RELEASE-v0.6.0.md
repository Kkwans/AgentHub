...
- Workspace 已提供 Local Project Terminal dock：能力可用时通过官方 `xterm.js` 连接既有安全 Terminal API
  与 `terminal:<id>` topic；能力缺失时显示中文原因并禁用操作，不使用普通 Shell 模拟 PTY。Docker/Remote
  Terminal 不在 v0.6 范围。
...
| NAS Compose              | `agenthub:0.6.0-nas.5`，ARM64，revision `a6f5c16`，`running/healthy`，`192.168.5.110:3210`                                                                                                  |
...
完整 NAS 记录见 [`docs/qa/nas/2026-08-15-v06-live5/README.md`](qa/nas/2026-08-15-v06-live5/README.md)。
...
- NAS 当前 `linux/arm64` 缺少可加载的 `node-pty` native binding，`GET /api/v1/settings/capabilities` 如实返回
  `PTY_NATIVE_BINDING_UNAVAILABLE`；Workspace 会显示中文不可用原因并禁用 Terminal 操作，不伪称 PTY 已可用。
...
记录见 [`docs/qa/nas/2026-08-15-v06-live5/`](qa/nas/2026-08-15-v06-live5/)。

...
- PromptOS 版本/Skill 来源、创建者与 Binding 解析顺序统一使用中文 Presentation；解析顺序说明与服务端“数字越小越优先”保持一致。代码证据与 nas.34 发布记录见 [`docs/qa/v06-promptos-presentation/README.md`](qa/v06-promptos-presentation/README.md)。
...
- Session 创建的 model/mode 现在按 capability 渐进式展示：Agent 提供候选列表时使用下拉选择；没有候选时只读显示
...
- Remote Node 注册表单现在使用中文“授权目录”添加器：支持绝对路径即时校验、重复/数量限制、可移除 Chip，
  不改变后端 roots 安全校验和一次性注册码语义。
- Remote Node Worktree/Git 能力限制现在使用与版本无关的中文下一步提示；overlay 发布会先清理旧 server/web dist，
  避免历史 hashed assets 或旧文案残留。
- Web 稳定错误映射中的 Remote Node Git 提示已同步为当前能力边界，不再出现旧版本“暂不提供”措辞。
- 网络失败与非 JSON 响应统一在 Web API 层归一化为稳定中文错误；普通错误卡片不再直接展示浏览器原始异常，原始诊断仍只保留在服务端/诊断边界。
- HTTP 200 但响应体不是对象时同样按稳定 `HTTP_ERROR` 处理，避免 `null`/数组响应触发原生解析异常。
- WebSocket 客户端对 JSON、topic 和 event envelope 做运行时校验，畸形 frame 直接丢弃，不把解析异常升级成前端 uncaught error。
- 错误 envelope 缺失或错误对象非法时统一回退为稳定 `HTTP_ERROR`，不再在客户端读取 `error.code` 时触发二次异常。
...
| NAS Compose nas.52       | `agenthub:0.6.0-nas.52`，ARM64，revision `e00c668`，`running/healthy`，`192.168.5.110:3210`；Agent 连接失败中文错误边界、Task 移动端可读列宽、真实 NAS Chromium 七页四视口 console/page/request error 0、横向溢出 0、health/root HTTP 200、Compose config 和 server 工作目录 node-pty `READY` 通过                                                               |
...
- Runtime 设置入口 nas.24 升级前 Compose/.env/token 校验值和容器状态备份：`/volume2/Project/.agenthub/central/deployments/20260816T085653Z-pre-nas24/`；Compose SHA-256
  `0e3e92b7078a4a6cfde4fa8c5493539ffac0e238f1aa570ff689606a095f27ff`，当前 `.env` SHA-256
  `d6d0405801ac6622fa77e2703a5743b277b2729f276fa0fac4f5fdad404f708e`，browser-token 仅保留 hash
  `d1e3d6d77a351bd669f975c32b414d8c9cd581e2e8fe87a11a4e0a64290db087`。镜像 `agenthub:0.6.0-nas.24` image ID
  `sha256:9083ef1895d15c628044d605d5af13eea1b4572c173b0d43a005408fa637c59f`；容器 ID
  `b8b19fe759f3ed3db985b4c2d9784bd5255e8e645c50ce0b6143d4401ffe1675`，最终 `running/healthy`。完整记录见
  [`docs/qa/nas/2026-08-16-v06-live24/README.md`](qa/nas/2026-08-16-v06-live24/README.md)。
- Session 配置 nas.25 升级前 Compose/.env/token 校验值和容器状态备份：`/volume2/Project/.agenthub/central/deployments/20260816T092116Z-pre-nas25/`；Compose SHA-256
  `0e3e92b7078a4a6cfde4fa8c5493539ffac0e238f1aa570ff689606a095f27ff`，当前 `.env` SHA-256
  `88b705c4eddd7da80fce213bae924543256bd86cf264cb739bf5de34482c715d`，browser-token 仅保留 hash
  `d1e3d6d77a351bd669f975c32b414d8c9cd581e2e8fe87a11a4e0a64290db087`。镜像 `agenthub:0.6.0-nas.25` image ID
  `sha256:2343c1d25aee0060e0d9b7ccd2715b4014a462a9d1748199516fcade8ba388ad`；容器 ID
  `21ef57e7321691e5a2e3f3a0c7890665b82c84ff2c0c7b683b35035e7eca74bb`，最终 `running/healthy`。完整记录见
  [`docs/qa/nas/2026-08-16-v06-live25/README.md`](qa/nas/2026-08-16-v06-live25/README.md)。
- Workspace 运行状态 nas.26 升级前 Compose/.env/token 校验值和容器状态备份：`/volume2/Project/.agenthub/central/deployments/20260816T100058Z-pre-nas26/`；Compose SHA-256
  `0e3e92b7078a4a6cfde4fa8c5493539ffac0e238f1aa570ff689606a095f27ff`，当前 `.env` SHA-256
  `c8744e55c51f56a6309c44d30c3865090a363ce3139336feb970d1581fa2028c`，browser-token 仅保留 hash
  `d1e3d6d77a351bd669f975c32b414d8c9cd581e2e8fe87a11a4e0a64290db087`。镜像 `agenthub:0.6.0-nas.26` image ID
  `sha256:5b044df0c7a49584ae31b828ec6ca781daf7ae6cf6ec6eb73a03509d4c8df3d0`；容器 ID
  `ebf9032ccf010a7b5d4be404dce71db7aa52e52d0745239a4e1e3367d8a00d47`，最终 `running/healthy`。完整记录见
  [`docs/qa/nas/2026-08-16-v06-live26/README.md`](qa/nas/2026-08-16-v06-live26/README.md)。nas.25 镜像保留作为回滚点。
...
- nas.52 已完成 NAS 本地 Playwright 自动化视觉 gate，作为当前唯一正式视觉门禁。升级前备份为
  `/volume2/Project/.agenthub/central/deployments/20260817T061748Z-pre-nas52/`；nas.51 镜像保留，可按单 service 回滚。升级使用 nas.51 overlay 仅覆盖 server/web dist 并保留 node-pty native binding；升级只重建 `agenthub` service，未执行 `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器。
...
- 当前唯一正式视觉门禁以 NAS 本地 Playwright Chromium 对真实 Compose 部署的 1440、1024、768、390 四视口截图、console/页面错误、横向溢出和关键交互断言为准；不依赖 TX5Pro、外部设备、人工浏览器或 fixture；nas.52 已完成该门禁。

...
- 软件版本：`1.1.0`；当前 PinHarness 直接迁移生产候选：`agenthub:2026.9.9-v16`，镜像为 Linux `arm64`，OCI `revision` 为 `f2c7b665ef9bae03c633ce66583e7c408485dd75`；
- 当前生产源码提交：`f2c7b665ef9bae03c633ce66583e7c408485dd75`（已推送 `main`）；镜像 ID 为 `sha256:f104a428cd713a5a3a2941eb04f156b7903d50e12f3836d568f009d96c164d5f`；
...
- 2026-09-09 10:30:04（Asia/Shanghai）已按用户明确授权替换生产 `agenthub` service。部署后 health `200` 返回版本 `1.1.0`，容器为 `running/healthy`，无 OOM/异常退出；Compose config 通过；
...
- 最新部署证据与回滚配置保存在 `/volume2/Project/.agenthub/central/deployments/20260909T022338Z-pre-direct-button-v16/`；v15 前候选材料保存在 `/volume2/Project/.agenthub/central/deployments/20260909T021404Z-pre-direct-button/`；未推送外部 registry，NAS 本地镜像已直接由 Compose 使用。

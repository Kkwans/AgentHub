...
- M24 Runtime 设置入口已完成：运行环境 discovery、接入、启动/停止和重新扫描抽取为共享
  `RuntimeDiscoveryPanel`，同时挂载到 Agent 页面与 `设置`，避免普通用户只能在 Agent 页面寻找高权限运行环境。
  Agent discovery 尚未返回时只显示本机或已接入目标，避免 Docker 容器扫描短暂加载期间把无关容器误展示；识别完成后
  才显示未识别容器数量和下一步提示。新增设置入口与加载边界 contract 测试；聚焦 Vitest 4/4、lint、typecheck、
  production build 通过，Playwright 四视口 24/24 通过（fixture，非人工视觉验收）。
- nas.24 已发布为 `agenthub:0.6.0-nas.24`（ARM64，image ID
  `sha256:9083ef1895d15c628044d605d5af13eea1b4572c173b0d43a005408fa637c59f`，revision `971f863`），
  容器 `b8b19fe759f3ed3db985b4c2d9784bd5255e8e645c50ce0b6143d4401ffe1675` 最终 `running/healthy`。
  `/api/v1/health`、根页面、Settings/Runtime 静态 bundle、授权 capability、node-pty 和 Terminal open/input/resize/close
...
- M26 Workspace 运行状态切片已完成：Session 的 `READY`、运行中、等待批准、取消中、断线、失败和已关闭状态
  统一映射为中文状态横幅；断线/已关闭状态提供回到 Session 列表的入口，非 `READY` Session 的 Composer
  禁止继续输入或发送，避免普通用户把已失效会话误当成可用会话。新增状态解析与 Workspace 回归测试。
- M26 自动化回归：非沙箱全仓 Vitest 通过 51 个文件、4 个 live 文件跳过，`219 passed / 9 skipped`；
  `lint`、`typecheck`、production build（Web 1716 modules transformed）通过；Playwright fixture 四视口
  `24/24` 通过，仍不替代人工视觉验收。对应代码提交为 `939d3cc`，已推送 `origin/main`。
- nas.26 已发布为 `agenthub:0.6.0-nas.26`（ARM64，image ID
  `sha256:5b044df0c7a49584ae31b828ec6ca781daf7ae6cf6ec6eb73a03509d4c8df3d0`，revision `939d3cc`），
  容器 `ebf9032ccf010a7b5d4be404dce71db7aa52e52d0745239a4e1e3367d8a00d47` 最终 `running/healthy`。
  `/api/v1/health`、根页面、Workspace 状态静态 bundle、授权 capability、node-pty 和 Terminal
  open/input/resize/close smoke 均通过；受保护 Agent 容器 name、完整 ID、image、状态保持发布前基线不变。
  升级前备份位于 `/volume2/Project/.agenthub/central/deployments/20260816T100058Z-pre-nas26/`，完整记录见
  `docs/qa/nas/2026-08-16-v06-live26/`。仅执行 `docker compose up -d --no-build agenthub`，未执行
  `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器；`.tmp-v05` 不存在。
- M27 用户可见状态收口已完成：Approval 决策卡片按“Agent 请求 / 影响 / 可选操作 / 投递结果”分层，原始错误
  只在诊断折叠区显示；Session 列表状态点复用 Workspace 状态解析；Task/Worktree 错误与 Runtime Docker
  状态统一提供中文下一步。代码提交为 `3e62e6c`、`f123b1a`、`9d3271a`、`3ed6455`、`018a03b`，均已推送。
- nas.28 已发布为 `agenthub:0.6.0-nas.28`（ARM64，image ID
  `sha256:da15594462db9e5f07f829f48a677b2e0fb15f6c3c0575241ac4ca7f19323df8`，revision `018a03b`），容器
  `98f3cd8753fc` 最终 `running/healthy`。health、根页面、node-pty 和运行 bundle 文案检查通过；升级前备份位于
  `/volume2/Project/.agenthub/central/deployments/20260816T103844Z-pre-nas28/`，完整记录见
  `docs/qa/nas/2026-08-16-v06-live28/`。仅执行 `docker compose up -d --no-build agenthub`，未执行
  `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器。
- M28 Remote Node 注册体验已收口：注册码表单将多行 `allowed roots` 改为中文“授权目录”添加器，支持绝对路径即时校验、重复/数量限制、Chip 展示和单项移除；后端注册协议与安全校验保持不变。代码提交为 `ebb49ee`、`70c6095`，均已推送。
- nas.29 已发布为 `agenthub:0.6.0-nas.29`（ARM64，image ID
  `sha256:70f975d75ae223ba926665d2ee50f63945fec60c9f1a9b798e50a1a1b5a04fc1`，revision `70c6095`），容器
...
- M31 Remote Node Git 错误映射已提交为 `de4ecd7`：移除 Web 稳定错误表中的旧“暂不提供”版本语义，并补充版本无关的能力边界回归。
...
- M33 Workspace Composer 可访问性修复已提交为 `b4ec7d0` 并发布 nas.33：消息输入框补齐 `aria-label`、`name` 与 `autocomplete="off"`；Workspace 聚焦回归 11/11、lint、typecheck、build 和 NAS health/root/node-pty smoke 通过。完整记录见 `docs/qa/nas/2026-08-16-v06-live33/`。
- M34 PromptOS Presentation 收口已提交为 `8adb2a5`：版本/Skill 来源、创建者与 Binding 解析顺序统一通过中文 Presentation 映射；同时修正“数字越小越优先”的表单说明。聚焦测试 2 files/4 tests、lint、typecheck、build 通过；随后由 M35 发布到 NAS。完整记录见 `docs/qa/v06-promptos-presentation/`。
- M35 PromptOS Presentation 已发布 nas.34：从 nas.33 运行时导出无 ACL/xattr 的单层 ARM64 镜像 `agenthub:0.6.0-nas.34`，规避 daemon `max depth exceeded`；revision `8adb2a5`，image digest `sha256:dfbe0fe3afb6e3e18851037653cdb2ebc3f274aff4c0467c0b8c8096f7dfbcf1`，容器 `running/healthy`。health/root、静态 bundle 文案和 server 工作目录 `node-pty` smoke 通过。升级前备份位于 `/volume2/Project/.agenthub/central/deployments/20260816T121812Z-pre-nas34/`；未执行 `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器。完整记录见 `docs/qa/v06-promptos-presentation/`。
- M36 UI surface symmetry 已提交为 `cd4c606` 并发布 nas.35：移除 `warning-surface` 单边强调条，统一控制层圆角/阴影；feature-boundary 9/9、lint、typecheck、build 通过。nas.35 容器 `running/healthy`，health/root HTTP 200、bundle 样式契约和 server 工作目录 `node-pty` smoke 通过。升级前备份位于 `/volume2/Project/.agenthub/central/deployments/20260816T124717Z-pre-nas35/`；未执行 `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器。完整记录见 `docs/qa/nas/2026-08-16-v06-live35/`。
- M37 Focus style 已提交为 `ac20416` 并发布 nas.36：删除旧蓝色 Radix focus fallback，统一按钮/链接与输入控件焦点 token；feature-boundary 9/9、lint、typecheck、build 通过。nas.36 容器 `running/healthy`，health/root HTTP 200、bundle 样式契约和 server 工作目录 `node-pty` smoke 通过。升级前备份位于 `/volume2/Project/.agenthub/central/deployments/20260816T125608Z-pre-nas36/`；未执行 `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器。完整记录见 `docs/qa/nas/2026-08-16-v06-live36/`。
- M38 普通 Agent 表面已提交为 `42bf4a2` 并发布 nas.37：普通 Agent 发现页与 Remote Node inventory 隐藏 adapter 实现细节，保留类型、版本、状态和诊断入口；Discovery/feature-boundary 2 files、11/11，lint、typecheck、build 通过。nas.37 容器 `running/healthy`，health/root HTTP 200、Compose config 和 server 工作目录 `node-pty` smoke 通过。升级前备份位于 `/volume2/Project/.agenthub/central/deployments/20260816T130838Z-pre-nas37/`；未执行 `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器。完整记录见 `docs/qa/nas/2026-08-16-v06-live37/`。
- M39 概览 Project 仓库类型展示已提交为 `a935a0f` 并发布 nas.38：将 `repoKind` 映射为“Git / 非 Git”，新增 feature-boundary 契约；Overview/feature-boundary 2 files、14/14，lint、typecheck、build 通过。nas.38 容器 `running/healthy`，health/root HTTP 200、Compose config 和 server 工作目录 `node-pty` smoke 通过。升级前备份位于 `/volume2/Project/.agenthub/central/deployments/20260816T133714Z-pre-nas38/`；未执行 `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器。完整记录见 `docs/qa/nas/2026-08-16-v06-live38/`。

- M40 用户错误边界已提交为 `cbd3044` 并发布 nas.39：统一 API 层归一化网络失败和非 JSON 响应，ErrorState/InlineError 不再向普通用户暴露浏览器原始异常；新增 2 项 API 回归测试。聚焦测试 4 files/18 tests、lint、typecheck、build 通过。nas.39 容器 `running/healthy`，health/root HTTP 200、Compose config 和 server 工作目录 `node-pty` smoke 通过。升级前备份位于 `/volume2/Project/.agenthub/central/deployments/20260816T135155Z-pre-nas39/`；未执行 `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器。完整记录见 `docs/qa/nas/2026-08-16-v06-live39/`。
...
- M43 错误信封边界已提交为 `b3d1cac` 并发布 nas.42：`error: null` 或缺少稳定 `code` 的响应统一降级为 `HTTP_ERROR`，避免客户端读取错误字段时抛出二次异常；API/realtime 聚焦 2 files/8 tests、typecheck、build 通过。nas.42 容器 `running/healthy`，health/root HTTP 200、Compose config 和 server 工作目录 `node-pty` smoke 通过。升级前备份位于 `/volume2/Project/.agenthub/central/deployments/20260816T141924Z-pre-nas42/`；未执行 `compose down`，未删除镜像、卷、用户数据或其他 Agent 容器。完整记录见 `docs/qa/nas/2026-08-16-v06-live42/`。
...
- M47 Agent 发现准确性已提交为 `869fe52` 并发布 nas.47：移除 `claw` 宽泛容器提示，避免把 `clawdash` 误识别为 OpenClaw；discovery focused test 10/10、server build、health/root 和 `node-pty=READY` 通过。升级前备份位于 `/volume2/Project/.agenthub/central/deployments/20260817T043319Z-pre-nas47/`；未执行 `compose down`，未删除镜像、卷、用户数据或其他 Agent 数据。
...
- 自动化视觉 gate 已完成：真实 nas.48 Agent 页面在 Chromium headless 下通过 1440/1024/768/390 稳定截图、console 和 390px 横向溢出检查；前一轮六页全量检查仍记录在 nas.45 视觉证据中。TX5Pro/Computer Use 手工通道仍不可用，因此仅保留 `TX5PRO_MANUAL_PENDING`，不伪称人工通过。

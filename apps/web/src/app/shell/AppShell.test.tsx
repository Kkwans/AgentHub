...
describe('v0.7 AppShell', () => {
  it('keeps Project as the context anchor and exposes the new primary IA', () => {
    ...
    expect(screen.getByRole('link', { name: 'Agent' }).getAttribute('href')).toBe('/agents/agents');
    expect(screen.getByRole('link', { name: 'Prompt Library' }).getAttribute('href')).toBe('/prompts');
